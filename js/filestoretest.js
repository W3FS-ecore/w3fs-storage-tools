//使用方法:0
//node filestoretest.js [w3fs_rpc] [user_address] [user_priv][oriHash] [nodeId] [fileHash] [bodyHash] [chain_id]
...
// const FileStoreStorageContract = require('./build/contracts/FileStoreStorage')
// const FileStoreProxyContract = require('./build/contracts/FileStoreProxy')
const FileStoreLogicContract = require('../build/contracts/FileStoreLogic')
// const fsstorage_abi = FileStoreStorageContract["abi"]
// const fsproxy_abi = FileStoreProxyContract["abi"]
...
const headHash=args[5]; //filehash
const fileBody=args[6]; //filehash2
const chainId=args[7]; //chaind_id
var fileSize = '1234';
...
// async function getLogicProxyInfo(){
//     await getLogicProxyContract.methods.createFileStoreInfo(oriHash,fileSize,nodeId,headHash,fileBody).send({from: user_address}),(function(result,error){
//         if(error) {
//             console.log(error)
//             return
//           }
...
const App = {
	...
	inittxObject: async function(){
		...
		return {
			...
			chainId:chainId,
			data: getLogicProxyContract.methods.createFileStoreInfo(this.oriHash,this.fileSize,this.fileExt,this.nodeId,this.headHash,this.fileBody).encodeABI()
		}
	},
	sendsign : async function () {
		let txObject = await this.inittxObject(this.user_address,this.oriHash,this.fileSize,this.nodeId,this.headHash,this.fileBody);
